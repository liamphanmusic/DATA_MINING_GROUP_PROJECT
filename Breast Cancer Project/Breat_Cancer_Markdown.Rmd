...
author: "Manuela Giansante, Liam Phan, Michael Bigler"
date: "2022-12-19"
output:  
  rmdformats::robobook:
    self_contained: true
...
knitr::opts_chunk$set(echo = TRUE,
                      cache = TRUE, warning = FALSE, message = FALSE, fig.align='center')
...
[Uci.edu. (2019). UCI Machine Learning Repository: Breast Cancer Wisconsin (Diagnostic) Data Set.](https://archive.ics.uci.edu/ml/datasets/Breast+Cancer+Wisconsin+(Diagnostic))
...
> Comments: We can see that there is no incoherence inside our variables range and type of all features seem adequate. They all are integer except **diagnosis** which is character and need to be formated in further part of this analysis.
...
> Comments: There is no misssing values in our datasaet, as showed by this plot
...
> Comments: Most variables are normally distributed except some features are right skewed. mostly in SE or Worst scenarios.

## Correlation

```{r corrplot, fig.height=16, fig.width=16}
...
> Comments: Correlation Plot shows us risk of multicolinearity and need to be adressed in some cases, such as when the model require explanatory analysis (ANOVA or others Statistical Test).

## Pairs plots

### predictors means plots: 

```{r Pairs Plots , fig.height= 10, fig.width= 10}
...
> Comments: We need to transform our outcome variable (or response variable) as factor of two levels, 0 for Begnign and 1 for Malign (Positive Case in our confusion matrix). 
...
> Comments: This output help us checking that our partitioning went correctly.
...
[What is supervised learning? - IBM](https://www.ibm.com/cloud/learn/supervised-learning#:~:text=Supervised%20learning%2C%20also%20known%20as,data%20or%20predict%20outcomes%20accurately.)
...
Test_Logistic <- Test_Logistic[,-c("id")]
...
> Comments: We can see that this model is very good in sensitivity and overall accuracy. 
...
```{r LR Predictions and Confusion Matrix Test}
...
# Plot the Confusion Matrix
draw_confusion_matrix_Logistic_Lasso_Test(Confusion_Matrix_Logistic_Lasso_Test)

```

> Comments: We can see that this model is very good in sensitivity and overall accuracy but perform better on the Test partition. 
...
```{r Logistic Regression Best Model Validation}
...
```{r Logistic Regression Best Model Test}
...
DF_Best_Logistic_Confusion_Test <- t(DF_Best_Logistic_Confusion_Test) # FINAL CONFUSION DATAFRAME
...
> Comments: The fully grown tree is quite reduced in size, still we proceed with the pruning. The Best Pruned tree has in total 5 end notes, obtained by selecting the minimum `xerror`.
We also report the table of variables in importance.
...
> Comments: Sensitivity lower than Specificity ( Malign diagnosis is minority). Accuracy amounts to 0.906, which is expected for such a simple model.
...
> Comments: Specificity is higher than Sensitivity, the model performs better in the categorisation of benign cases.
...
As K nearest neighbour is based on distances the predictors need to be standardized such that they have a mean at 0 and a variance equal to 1. This necessity of standardization is due to the fact that otherwise variables with bigger values have more influence on the distance that is being calculated. 
...
To get the best k one might iterate over several k and choose the one which has the highest value for the metric that is being looked at. In this case we looked at either accuracy or sensitivity. 
...
```{r best knn in relativity to sensitivity k equal 1}
set.seed(1)
# nearest neighbor
...
draw_confusion_matrix(KNN1.conf.mat.test, titleaddon = "KNN for test data with k=6")
...
DF_Best_KNN_Confusion_test <- t(DF_Best_KNN_Confusion_test) # FINAL CONFUSION DATAFRAME
```
...
Neural_1_Plot <- plot(Neural_1)
Neural_1_Plot

```


![Basic Neural Network with 3 Hidden Nodes and 1 Layer](IMAGES\LIAM\NEURAL NETWORK\BASIC NEURAL NETWORK - 3 HIDDEN NODES and 1 LAYER.png)
...
Neural_2_Plot <- plot(Neural_2)
Neural_2_Plot

```

![Advanced Neural Network with 15 Hidden nodes and 1 layer](IMAGES\LIAM\NEURAL NETWORK\ADVANCED NEURAL NETWORK - 15 HIDDEN NODES and 1 LAYER.png)
...
> Comments: Best Neural Network Model both Sensitivity and Accuracy.
...
```{r Predictions and Confusion Matrix for NN 2.2}

set.seed(1)

# Predictions 
Predictions_NN2_Test <- predict(Neural_2, Test_NN_Preprocess, type="response")
Predictions_NN2_Probabilities_Test <- Predictions_NN2_Test[,2]

# Rounding Predictions - 0.5 Threshold 
Predictions_NN2_Dummy_Test <- round(Predictions_NN2_Probabilities_Test)

# As Numeric
Predictions_NN2_Dummy_Test <- as.numeric(Predictions_NN2_Dummy_Test)

# Check rounding in a Dataframe
DF_Neural_Predictions_2_Test <- cbind(Predictions_NN2_Probabilities_Test, Predictions_NN2_Dummy_Test)

# As Factor
Predictions_NN2_Dummy_Test <- factor(Predictions_NN2_Dummy_Test)
Test_NN_Preprocess$diagnosis <- factor(Test_NN_Preprocess$diagnosis)

# Confusion Matrix
Confusion_Matrix_Neural_2_Test <- confusionMatrix(data = Predictions_NN2_Dummy_Test, reference = Test_NN_Preprocess$diagnosis, positive = "1")

# Create the Function for Confusion Matrix
draw_confusion_matrix_Neural_2_Test <- function(cm) {
...
# Plot the Confusion Matrix
draw_confusion_matrix_Neural_2_Test(Confusion_Matrix_Neural_2_Test)
...
> Comments: Very stable and can perform better on Test so no overfitting occured.
...
# Plot Neural Network Model 2
Neural_3_Plot <- plot(Neural_3)
Neural_3_Plot

```

![Advanced Neural Network with 15 Hidden nodes and 2 layers](IMAGES\LIAM\NEURAL NETWORK\ADVANCED NEURAL NETWORK - 15 HIDDEN NODES and 2 LAYER.png)
...
> Comments: Not better than the second model but better than the Basic Neural Network.
...
```{r Neural Network Best Model Validation}
...
```{r Neural Network Best Model Test}
...
DF_Best_Neural_Confusion_Test <- t(DF_Best_Neural_Confusion_Test) # FINAL CONFUSION DATAFRAME
...
> Comments: Not a bad model but again it lacks sensitivity. As the model is quite big we can try a variable selection. 

```{r lda variable selection}
set.seed(1)
...
> Comments: We see that the model only drops one variable and the predictive power of the model doesn't change much as instead of 1 false positive and 7 false negatives there are now 0 false positives and 8 false negatives. 
...
> Comments: Overall this model is worse than the lda but i case of sensitivtiy it is better as there are only 4 false negatives. 
...
> Comments: This time around 6 variables were dropped but we see that the model is not better than the qda with all predictors. 
...
> Comments: We see that this method in general works better than the lda and qda. In this case we don't proceed with this model as it isn't better in sensitivity which is what we want when predicting cancer. 
...
> Comments: We see that the model has a good accuracy but isn't good in sensitivity. Therefore we don't use it further. 
...
draw_confusion_matrix(DA2.conf.mat.test, titleaddon = 'Quadratic Discriminant Analysis')
...
DF_Best_DA_Confusion_test <- t(DF_Best_DA_Confusion_test) # FINAL CONFUSION DATAFRAME
...
> Comments: The model performing best is the Boosted Trees model. They all provide improved classification capacity than the orginal Best Pruned Tree.
...
```{r Best Model - Boosting Trees on Validation}
...
> Comments: We have reported the best model from the Trees family.

### Best Model - Boosting Tree on Test

```{r Best Model- Boosting Trees on Test}
...
draw_confusion_matrix(Confusion_Boosted_Test)
...
DF_Boosted_Confusion_Test<- t(DF_Boosted_Confusion_Test)

```

> Comments: We predict the performance of Boosted Trees on the test set so to then recompute the Average Ensemble. The model seems to performs better n the test set than how it behaved over the validation set.
...
# Best Models
## Best 7 Models - On Validation

```{r Best 5 Models amongst all, results='hide'}
...
DF_Best_Neural_Network_Predictions[,2] <- as.numeric(DF_Best_Neural_Network_Predictions[,2])
...
```{r Average of Models Probabilities}
...
### Confusion Matrix of Majority and Average

```{r Confusion Matrix of Majority and Average}
...
draw_confusion_matrix_Majority_Validation(Majority_confusion)
...
draw_confusion_matrix_Average_Validation(Average_confusion)

# Majority Confusion as Data frame
...
DF_Average_Confusion <- t(DF_Average_Confusion) # FINAL CONFUSION DATAFRAME
...
DT::datatable(round(TABLE_7_MODELS,4), caption = "7 Models on Validation - Order by Sensitivity") 

```

> Comments: 3 Best Models according to both **Sensitivity** and **Accuracy** are: **Neural Network**, **DA** and **Average Probabilities** on Validation. 


## Best 7 Models - On Test

```{r On Test Gathering, results='hide'}
...
DF_Best_Neural_Network_Predictions_Test[,2] <- as.numeric(DF_Best_Neural_Network_Predictions_Test[,2])
...
DT::datatable(Majority_DF_Test, caption = "Best 5 Models on Test - Majority of Votes") 
...
```{r Average of Models Probabilities Test}
...
DT::datatable(round(Average_DF_Test,4), caption = "Best 5 Models on Test - Average of Probabilities") 
...
```{r Confusion Matrix of Majority and Average Test}
...
draw_confusion_matrix_Majority_Test(Majority_confusion_Test)
...
draw_confusion_matrix_Average_Test(Average_confusion_Test)
...
DF_Average_Confusion_Test <- t(DF_Average_Confusion_Test) # FINAL CONFUSION DATAFRAME
...
DT::datatable(round(TABLE_7_MODELS_Test,4), caption = "7 Models on Test - Order by Sensitivity") 
...
## Best 3 Models After Validation and Test

The Best 3 Models after all the process done in Validation and Test are:
...
> Comments: Ranking based on **Sensitivity** First and **Accuracy** second.
...
[What is unsupervised learning? - IBM](https://www.ibm.com/cloud/learn/unsupervised-learning#:~:text=Unsupervised%20learning%2C%20also%20known%20as,the%20need%20for%20human%20intervention.)
...
[Classification: LDA and QDA Approaches](https://pages.mtu.edu/~shanem/psy5220/daily/Day12/classification.html#:~:text=Variable%20Selection%20in%20LDA,-We%20now%20have&text=The%20best%20models%20have%201,the%20cross%2Dvalidation%20is%20initialized.)

[Discriminant Analysis Essentials in R](http://www.sthda.com/english/articles/36-classification-methods-essentials/146-discriminant-analysis-essentials-in-r/)

[rmdformats](https://juba.github.io/rmdformats/)

[Confusion Plot Function](https://stackoverflow.com/questions/23891140/r-how-to-visualize-confusion-matrix-using-the-caret-package)
