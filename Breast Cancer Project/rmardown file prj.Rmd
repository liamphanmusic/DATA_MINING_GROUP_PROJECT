...
Training_NN <- as.data.frame(Training_NN)
Validation_NN <- as.data.frame(Validation_NN)

# Remove the "ID" Variable
Training_NN <- Training_NN[,-c("id")]
Validation_NN <- Validation_NN[,-c("id")]
...
# Factor Diagnosis Variable
Training_NN_Preprocess$diagnosis <- factor(Training_NN_Preprocess$diagnosis)
Validation_NN_Preprocess$diagnosis <- factor(Validation_NN_Preprocess$diagnosis)

# Fit neural network with 3 hidden layers
Neural_1 <- neuralnet(Training_NN_Preprocess ~ ., data = Training_NN_Preprocess, hidden=3, linear.output = FALSE)
