...
# Confusion Matrix
Confusion_Matrix_Logistic_Lasso <- confusionMatrix(data = Logistic_Lasso_Predictions_Dummy, reference = Validation_Logistic$diagnosis)
...
# Predictions with LR
Logistic_Lasso_Predictions_2 <- predict(Logistic_Model_After_VIF4, Test[,c("area_mean", "smoothness_mean", "symmetry_mean", "smoothness_se", "concave points_se", "symmetry_se", "concave points_worst", "fractal_dimension_worst")], type = "response")

# Rounding Predictions - 0.5 Threshold
Logistic_Lasso_Predictions_Dummy_2 <- round(Logistic_Lasso_Predictions_2)

# As Numeric
Logistic_Lasso_Predictions_Dummy_2 <- as.numeric(Logistic_Lasso_Predictions_Dummy_2)

# Check rounding in a Dataframe
DF_Logistic_Lasso_Predictions_2 <- cbind(Logistic_Lasso_Predictions_2, Logistic_Lasso_Predictions_Dummy_2)

# As Factor
Logistic_Lasso_Predictions_Dummy_2 <- as.factor(Logistic_Lasso_Predictions_Dummy_2)

# Confusion Matrix
Confusion_Matrix_Logistic_Lasso_2 <- confusionMatrix(data = Logistic_Lasso_Predictions_Dummy_2, reference = Test$diagnosis)

# Create the Function for Confusion Matrix
draw_confusion_matrix_Logistic_Lasso_2 <- function(cm) {
...
# Plot the Confusion Matrix
draw_confusion_matrix_Logistic_Lasso_2(Confusion_Matrix_Logistic_Lasso_2)

```

> Comments: 

#### ROC Curve of the Logistic Regression - Test

```{r ROC Curve Lasso Test}
...
# Plot our ROC Curve
pr2 <- ROCR::prediction(Logistic_Lasso_Predictions_2, Test$diagnosis)
prf2 <- ROCR::performance(pr, measure = "tpr", x.measure = "fpr")
plot(prf2, main="ROC for Test Set")
abline(a = 0, b = 1) 
...
# Confusion Matrix
Confusion_Matrix_Neural_1 <- confusionMatrix(data = Predictions_NN1_Dummy, reference = Validation_NN_Preprocess$diagnosis)
...
```{r Predictions and Confusion Matrix for NN 1 Test}

set.seed(1)

# Predictions
Predictions_NN1_Test <- predict(Neural_1, Test_NN_Preprocess, type="response")
Predictions_NN1_Probabilities_Test <- Predictions_NN1_Test[,2]

# Rounding Predictions - 0.5 Threshold
Predictions_NN1_Dummy_Test <- round(Predictions_NN1_Probabilities_Test)

# As Numeric
Predictions_NN1_Dummy_Test <- as.numeric(Predictions_NN1_Dummy_Test)

# Check rounding in a Dataframe
DF_Neural_Predictions_Test <- cbind(Predictions_NN1_Probabilities_Test, Predictions_NN1_Dummy_Test)

# As Factor
Predictions_NN1_Dummy_Test <- factor(Predictions_NN1_Dummy_Test)
Test_NN_Preprocess$diagnosis <- factor(Test_NN_Preprocess$diagnosis)

# Confusion Matrix
Confusion_Matrix_Neural_1_Test <- confusionMatrix(data = Predictions_NN1_Dummy_Test, reference = Test_NN_Preprocess$diagnosis)

# Create the Function for Confusion Matrix
draw_confusion_matrix_Neural_1_Test <- function(cm) {
...
# Plot the Confusion Matrix
draw_confusion_matrix_Neural_1_Test(Confusion_Matrix_Neural_1_Test)

```

> Comments: Better Result compared to Model 3 in Test, but low Specifity. 
...
# Confusion Matrix
Confusion_Matrix_Neural_2 <- confusionMatrix(data = Predictions_NN2_Dummy, reference = Validation_NN_Preprocess$diagnosis)
...
> Comments: Good Specificity and Accuracy

##### Confusion Matrix with Test

<center>

```{r Predictions and Confusion Matrix for NN 2 Test}
...
# Confusion Matrix
Confusion_Matrix_Neural_2_Test <- confusionMatrix(data = Predictions_NN2_Dummy_Test, reference = Test_NN_Preprocess$diagnosis)
...
> Comments: Model 2 best Specificity and Accuracy same as Model 1 on Test, meaning more stable but more accurate for predicting Malignant Tumors. We would choose this model. 
...
# Plot our ROC Curve
pr2_NN_Test <- ROCR::prediction(Predictions_NN2_Probabilities_Test, Test_NN_Preprocess$diagnosis)
prf2_NN_Test <- ROCR::performance(pr2_NN_Test, measure = "tpr", x.measure = "fpr")
plot(prf2_NN_Test, main="ROC for Test Set")
abline(a = 0, b = 1) 
...
# Confusion Matrix
Confusion_Matrix_Neural_3 <- confusionMatrix(data = Predictions_NN3_Dummy, reference = Validation_NN_Preprocess$diagnosis)
...
> Comments: Good Predictions, but need to check overfitting in Test

##### Confusion Matrix For Test

<center>

```{r Predictions and Confusion Matrix for NN 3 Test}

set.seed(1)

# Predictions 
Predictions_NN3_Test <- predict(Neural_3, Test_NN_Preprocess, type="response")
Predictions_NN3_Probabilities_Test <- Predictions_NN3_Test[,2]

# Rounding Predictions - 0.5 Threshold 
Predictions_NN3_Dummy_Test <- round(Predictions_NN3_Probabilities_Test)

# As Numeric
Predictions_NN3_Dummy_Test <- as.numeric(Predictions_NN3_Dummy_Test)

# Check rounding in a Dataframe
DF_Neural_Predictions_3_Test <- cbind(Predictions_NN3_Probabilities_Test, Predictions_NN3_Dummy_Test)

# As Factor
Predictions_NN3_Dummy_Test <- factor(Predictions_NN3_Dummy_Test)
Test_NN_Preprocess$diagnosis <- factor(Test_NN_Preprocess$diagnosis)

# Confusion Matrix
Confusion_Matrix_Neural_3_Test <- confusionMatrix(data = Predictions_NN3_Dummy_Test, reference = Test_NN_Preprocess$diagnosis)

# Create the Function for Confusion Matrix
draw_confusion_matrix_Neural_3_Test <- function(cm) {
...
# Plot the Confusion Matrix
draw_confusion_matrix_Neural_3_Test(Confusion_Matrix_Neural_3_Test)

```

</center>

> Comments: Overfitting with 2 Layers and 15 Nodes - Less accurate predictions compared to Model 1 and 2. 
...
draw_confusion_matrix(DA3.conf.mat)

# Lift chart
actual <- as.numeric(as.character(ORIGINAL.DA.valid$diagnosis))
prob <- as.numeric(DA3.pred.valid.prob)
DA3.gain <- gains(actual, prob)
barplot(DA3.gain$mean.resp / mean(actual), names.arg = DA3.gain$depth, xlab = "Percentile",
ylab = "Mean Response", main = "Decile-wise lift chart")
...
draw_confusion_matrix(DA4.conf.mat)

# Lift chart
actual <- as.numeric(as.character(ORIGINAL.DA.valid$diagnosis))
prob <- as.numeric(DA4.pred.valid.prob)
DA4.gain <- gains(actual, prob)
barplot(DA4.gain$mean.resp / mean(actual), names.arg = DA4.gain$depth, xlab = "Percentile",
ylab = "Mean Response", main = "Decile-wise lift chart")
```
...
draw_confusion_matrix(DA5.conf.mat)

# Lift chart
actual <- as.numeric(as.character(ORIGINAL.DA.valid$diagnosis))
prob <- as.numeric(DA5.pred.valid.prob)
DA5.gain <- gains(actual, prob)
barplot(DA5.gain$mean.resp / mean(actual), names.arg = DA5.gain$depth, xlab = "Percentile",
ylab = "Mean Response", main = "Decile-wise lift chart")
```
...
bag_pred<- predict(bagging, Validation, type="class")
bag_cf <- confusionMatrix(as.factor(bag_pred$class), Validation$diagnosis, positive = "1")
...
boost_pred<- predict(boosting, Validation, type="class")
boost_cf <- confusionMatrix(as.factor(boost_pred$class), Validation$diagnosis, positive = "1")
...
rf_pred<- predict(rand_f, Validation, type="class")
rf_cf <- confusionMatrix(as.factor(rf_pred), Validation$diagnosis, positive = "1")
...
```{r}
 # For the boosting
response_boost <- data.frame(Validation[,1], boost_pred$class) 
response_boost$Validation...1.<- as.numeric(as.character(response_boost[,1]))
response_boost$boost_pred.class<- as.numeric(as.character(response_boost[,2]))
roc_score_boost <- roc(data= response_boost , response=Validation...1., boost_pred.class) #AUC score


# For bagging
response_bag <- data.frame(Validation[,1], bag_pred$class) 
response_bag$Validation...1.<- as.numeric(as.character(response_bag[,1]))
response_bag$bag_pred.class<- as.numeric(as.character(response_bag[,2]))
roc_score_bag <- roc(data= response_bag , response=Validation...1., bag_pred.class) #AUC score


# For Random Forests
response_rf <- data.frame(Validation[,1], rf_pred) 
response_rf$Validation...1.<- as.numeric(as.character(response_rf[,1]))
response_rf$rf_pred<- as.numeric(as.character(response_rf[,2]))
roc_score_rf <- roc(data= response_rf , response=Validation...1., rf_pred) #AUC score
...
# Confusion Matrix
Confusion_Matrix_K_Means1 <- confusionMatrix(data = DF_Cluster_Performance$`Cluster_ALL$cluster`, reference = DF_Cluster_Performance$`KClusteringDF_Preprocess$diagnosis`)
...
We can see that we lack a lot of information from this dataset, we can only infer the size of the cancer based on 1 tumor, without nearby information. Thus we will be limited in the clustering method to only size. 
...
[Introduction to Neural Networks for Java (second edition) by Jeff Heaton - Google Books](https://books.google.it/books?id=Swlcw7M4uD8C&pg=PA158&lpg=PA158&dq=Introduction%20to%20Neural%20Networks%20for%20Java,%20Second%20Edition%20The%20Number%20of%20Hidden%20Layers&source=bl&ots=TJx9QaeWw6&sig=gZqg9e73K1oCqWBxmcBWAf2pbrE&hl=it&sa=X&ved=0CCUQ6AEwAGoVChMIudnOsJr1yAIVwjkaCh3AAgnU#v=onepage&q=Introduction%20to%20Neural%20Networks%20for%20Java%2C%20Second%20Edition%20The%20Number%20of%20Hidden%20Layers&f=false)
