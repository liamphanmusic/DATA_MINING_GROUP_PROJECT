...
```{r DA data prep}
ORIGINAL$diagnosis <- factor(ifelse(ORIGINAL$diagnosis == "M", 1, 0))
ORIGINAL.DA <- dplyr::select(ORIGINAL, -c("id"))
norm.value <- preProcess(ORIGINAL.DA, method = c("center", "scale"))
ORIGINAL.DA <- predict(norm.value, ORIGINAL.DA)
...
# confusion matrix
ct.conf.mat <- confusionMatrix(DA1.pred.valid.resp, ORIGINAL.DA.valid$diagnosis, positive = "1")
draw_confusion_matrix(ct.conf.mat)
