...
# Fit The Logistic Regression Model
Logistic_Model_2 <- glm(diagnosis ~ radius_mean , family=binomial(link='logit'), data = Training_Logistic)
...
splitting <- sample(1:3,size=nrow(ORIGINAL_M),replace=TRUE,prob=c(0.5,0.3,0.2))
Training <- ORIGINAL_M[splitting==1,]
Validation <- ORIGINAL_M[splitting==2,]
Test <- ORIGINAL_M[splitting==3,]
...
# Plot tree
rpart.plot(tree_full, yesno = TRUE, digits =-3)
...
kable(relevance, row.names = T,col.names="Importance")%>% kable_paper("hover", full_width = T) #built table
```
