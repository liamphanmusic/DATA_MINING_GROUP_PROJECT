...
sampling <- sample(row.names(ORIGINAL), 0.5*dim(ORIGINAL)) #  TRAINING set 50% of the data
sampling_2 <- sample(setdiff(row.names(ORIGINAL), sampling ),  0.3*dim(ORIGINAL)) # VALIDATION set 30%
sampling_3 <- setdiff(rownames(ORIGINAL), union(sampling, sampling_2)) #TEST set 20%
# we build all these sets as mutually exclusive

train.ORIGINAL <- ORIGINAL[sampling ,] # TRAINING set as individual data.frame
VALID <- ORIGINAL[sampling_2,] # VALIDATION set as individual data.frame
TEST.e.ORIGINAL <- ORIGINAL[sampling_3,] # TEST set as individual data.frame

train.estand.ORIGINAL <- train.ORIGINAL # initializing variables, for TRAINING set
valid.estand.ORIGINAL <-valid.ORIGINAL  # for VALIDATION set
TEST.estand.ORIGINAL <- TEST.ORIGINAL   # for TEST set
