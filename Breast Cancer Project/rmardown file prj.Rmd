...
ggplot(gather(ORIGINAL, key, value, -diagnosis), aes(value)) + 
    geom_histogram(bins = 10) + 
    facet_wrap(~key, scales = 'free_x')

# histogram over all columns grouped by diagnosis
ggplot(gather(ORIGINAL, key, value, -diagnosis), aes(value, fill = factor(diagnosis))) + 
...
#Re-anme teh partitions in the data
Training_M <- Training
...
response[,1]<- as.numeric(as.character(response[,1]))
response$pruned_pred<- as.numeric(as.character(response$pruned_pred))
roc_score <- roc(data= response , response=diagnosis, pruned_pred) #AUC score
...
sensitivity.df <- data.frame(k = seq(1, 30, 1), accuracy = rep(0, 30))
...
best k with accuracy and not overfitted (second highest sensitivity) k=6
...
draw_confusion_matrix(KNN1.conf.mat.test, titleaddon = 'KNN test')
```

highest sensitivity at k=1 --> overfitted

```{r best knn in relativity to sensitivity k=1}
set.seed(1)
# nearest neighbor
...
```{r best knn in relativity to sensitivity k=1 test}
# predictions response 
KNN2.pred.test.resp <- predict(KNN2, dplyr::select(ORIGINAL.KNN.test, -c(diagnosis)), type = "class")

# predictions prob 
KNN2.pred.test.prob <- predict(KNN2, dplyr::select(ORIGINAL.KNN.test, -c(diagnosis)), type = "prob")[,2]
# Confusionmatrix Test
KNN2.conf.mat.test <- confusionMatrix(KNN2.pred.test.resp, ORIGINAL.KNN.test$diagnosis, positive = "1")
draw_confusion_matrix(KNN2.conf.mat.test, titleaddon = 'KNN test')
```
...
```{r Best 5 Models amongst all, echo=FALSE}
...
Majority_DF <- data.frame(DF_Best_Logistic_Predictions[,1], DF_best_boosted_pred[,1], DF_Best_Neural_Network_Predictions[,1])

for(i in 1:nrow(Majority_DF)){
  if(sum(Majority_DF[i,1] + Majority_DF[i,2] + Majority_DF[i,3])/3 > 0.5){
    Majority_DF[i,4] = 1
    }else{Majority_DF[i,4] = 0}
}

colnames(Majority_DF) <- c("Logistic Regression", "Boosted Trees","Neural Network","Majority Vote")

Majority_DF$'Validation Actual' <- as.numeric(as.character(Validation$diagnosis))

DT::datatable(Majority_DF, caption = "Majority of Votes") 
...
Average_DF <- data.frame(DF_Best_Logistic_Predictions[,2], DF_best_boosted_pred[,2], DF_Best_Neural_Network_Predictions[,2])

for(i in 1:nrow(Average_DF)){
  Average_DF[i,4] <- sum(Average_DF[i,1] + Average_DF[i,2] + Average_DF[i,3])/3
}

colnames(Average_DF) <- c("Logistic Regression", "Boosted Trees","Neural Network","Average")

Average_DF$'Average Dummy' <- ifelse(Average_DF$`Average`>0.5,1,0)

Average_DF$'Validation Actual' <- as.numeric(as.character(Validation$diagnosis))

DT::datatable(Average_DF, caption = "Average of Models Probabilities") 
